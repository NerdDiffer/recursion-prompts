...
var arraySum = function(array) {
  ...
  if (Array.isArray(first)) {
    return arraySum(first) + arraySum(copy.slice(1));
  } else {
    var len = copy.length;
    ...
    return copy.shift() + arraySum(copy);
  }
};
...
var exponent = function(base, x) {
  if (x === 0) {
    return 1;
  } else if (x < 0) {
    return 1 / exponent(base, -x);
  } else {
    var y;
    var even = x % 2 === 0;
    ...
    if (even) {
      y = x / 2;
      return exponent(base, y) * exponent(base, y);
    } else {
      y = x - 1;
...
var palindrome = function(string) {
  ...
  // compare values at these indexes AND ignore case
  var same = string[a].toLowerCase() === string[b].toLowerCase();

  if (!same) { return false; }
  if (a === b && same) { return true; }

  return palindrome(string, a + 1);
};
